...
export default function Shifts() {
  ...
  useEffect(() => { loadShifts(); fetchExpenses(); }, []);

  async function loadShifts() {
    setLoading(true);
    const { data, error } = await supabase.from('shifts').select('*, users!shifts_cashier_id_fkey(full_name)').order('opened_at', { ascending: false }).limit(isOwner ? 50 : 10);
    if (error) { toast.error('Failed to load shifts'); } else { setShifts(data || []); const active = (data || []).find(s => s.cashier_id === user.id && s.status === 'open'); setActiveShift(active); }
    setLoading(false);
  }
  ...
  async function prepareOpenForm() {
    const { data: lastClosed } = await supabase.from('shifts').select('*').eq('status', 'closed').order('closed_at', { ascending: false }).limit(1).single();
    let suggested = 0;
    if (lastClosed) {
  ...
  async function openShift(e) {
    ...
    const { error } = await supabase.from('shifts').insert([{ cashier_id: user.id, shift_type: shiftType, opening_cash: amount, status: 'open', notes: shiftNotes || null }]);
    if (error) { toast.error(error.message); } else { toast.success(`${shiftType} shift opened`); setShowOpenForm(false); setSuggestedOpening(null); loadShifts(); }
  }
  ...
  async function prepareCloseForm() {
    ...
    if (tabIds.length === 0) {
      setCashSales(0); setExpectedClosing(activeShift.opening_cash); setClosingCash(''); setShowCloseForm(true); return;
    }
    const { data: cashPayments } = await supabase.from('payments').select('amount').eq('payment_method', 'cash').in('tab_id', tabIds);
  ...
  async function closeShift(e) {
    ...
    if (isNaN(actualClosing) || actualClosing < 0) { toast.error('Enter valid closing cash amount'); return; }
    const variance = actualClosing - expectedClosing;
    const { error } = await supabase.from('shifts').update({ closing_cash: actualClosing, expected_cash: expectedClosing, variance, status: 'closed', closed_at: new Date().toISOString() }).eq('id', activeShift.id);
    if (error) { toast.error(error.message); } else { toast.success(`Shift closed. Variance: UGX ${variance.toLocaleString()}`); setShowCloseForm(false); loadShifts(); }
  }
  ...
  async function handleAddExpense(e) {
    ...
    const { error } = await supabase.from('expenses').insert([{ amount, description: expenseDesc || null, recorded_by: user.id }]);
    if (error) { toast.error(error.message); } else { toast.success('Expense recorded'); setShowExpenseForm(false); setExpenseAmount(''); setExpenseDesc(''); fetchExpenses(); }
  }
  ...
  if (loading) { return <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 dark:border-blue-400"></div></div>; }
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-700 dark:text-blue-400 text-sm whitespace-nowrap">Opened: {new Date(activeShift.opened_at).toLocaleTimeString('en-UG', { hour: '2-digit', minute: '2-digit' })}</p>
            </div>
            ...
            <div className="p-6">
              ...
              <form onSubmit={closeShift} className="space-y-4">
                <div>
                  ...
                  {closingCash && <p className={`mt-2 text-sm font-medium ${parseFloat(closingCash) - expectedClosing >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>Variance: {formatCurrency(parseFloat(closingCash) - expectedClosing)}</p>}
                </div>
                <div className="flex gap-3">
